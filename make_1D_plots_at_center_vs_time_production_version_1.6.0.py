...
fout=open(od + "/energy_density_evolution_at_grid_center_scaled.dat","w")
sp="    "
#plt.title("Energy density in the center of the grid")
plt.xlabel('t/t$_{max}$')
plt.ylabel(r'$\varepsilon/\varepsilon_{max}$')
plt.xlim(0, 15)
i=0
for key, value in infiles.items():    
    idx_max=np.argmax(eps_dict[key])
    time_of_max=tt_dict[key][idx_max]
    energy_max=np.amax(eps_dict[key])
    plt.plot(tt_dict[key]/time_of_max, eps_dict[key]/energy_max, color=colors[i], linewidth=2, linestyle=linestyles[i], label=value[1])
    i=i+1
    if (i>1):
         fout.write("\n\n\n")
    fout.write("# "+value[1]+"\n")
    fout.write("# time of maximum: "+'{:6.4f}'.format(time_of_max)+"\n") 
    fout.write("# max of energy density: "+'{:7.5f}'.format(time_of_max)+"\n") 
    fout.write("# 1-time / time of max energy density       2-energy density / max of energy density\n\n")
    for mm in range(len(tt_dict[key][:])):
        if ((tt_dict[key][mm]>=time_start) and (tt_dict[key][mm]<=time_end)):
            fout.write('{:5.3f}'.format(tt_dict[key][mm]/time_of_max)+sp+'{:11.9e}'.format(eps_dict[key][mm]/energy_max)+"\n")
fout.close()
plt.minorticks_on()
plt.grid(b=True, color='#BBBBBB', linestyle='-')
plt.legend()
plt.tight_layout()
plt.savefig(od + "/energy_density_evolution_at_grid_center_scaled.png", dpi=300, pad_inches=0.)
plt.savefig(od + "/energy_density_evolution_at_grid_center_scaled.pdf", pad_inches=0.)
plt.close('all')
